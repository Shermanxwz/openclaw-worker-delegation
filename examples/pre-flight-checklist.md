...
A printable, expanded version of the pre-flight checklist in the README and `SKILL.md`. Walk through this before every spawn decision. If a box cannot be checked, fix it before spawning — the rest of the workflow assumes each box is true.
...
### 1.1 Identity of the active session

- [ ] I know which **model** the main session is currently on (from `session_status` or the runtime header).
...
### 1.3 Justification for the spawn (if any)

- [ ] The decision is **not** "I felt like it" or "the strong model is busy".
- [ ] If main is already on the worker model, the brief **explicitly** justifies the exception: parallelism, isolation, long background, or cleanly separable subproblem.
...
- [ ] The worker's actions are **inside Scope**.
- [ ] The worker has not crossed any line in **Do not** without explicit approval.
...
- [ ] The main session has not fallen back to the worker model mid-run. If it has, stop spawning same-model workers for serial work.

A short mental note ("worker is on <expected-model>", "main is on <expected-model>") at the start of each turn keeps this honest.
