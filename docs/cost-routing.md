...
A practical guide to confirming, at runtime, that the main session is actually using the model you think it is, and that worker delegation is producing the cost profile you expect.

The skill is small. Most of the time it works. When it doesn't, the failure usually looks like a cost surprise: a long run finishes and the bill doesn't match the rule. This doc is about how to catch that *before* the bill arrives.

> **Audience.** This doc assumes you are operating a session, not a billing admin. It is intentionally generic — no provider SDK calls, no private dashboards. Everything here is something you can do from inside the session.

## 1. What "verify routing" actually means

There are three distinct things to verify, and they are not the same:

1. **Identity of the active model.** Which model is the main session *actually* running on right now? Not which one you asked for.
2. **Identity of the worker model.** Which model did the worker run on? Not which one you put in the brief.
3. **Identity of the billing source.** Which model is your account actually being charged for, given the fallback chain the runtime is allowed to use?

These three are usually equal. When they are not, that's the source of almost every cost surprise in this category of workflow. Treat them as three separate things to verify.

## 2. Verifying the active model in the main session

Most runtimes expose one of:

- A `session_status` tool (or equivalent) that returns the current model and runtime metadata.
- A header in the runtime that the agent can read directly.
- A status block at the top of each turn.

The exact field name varies. Look for one of:
...
If none of these are present, the runtime is being secretive about which model is running. Treat that as a sign that cost is also being managed opaquely — escalate to a stronger guardrail before the run.

### What to do with the result

- The model name returned by the runtime is the **identity of the active model**, not necessarily the **identity of the billing source**. The runtime may have fallen back without telling the model, and the active model can be a different family from the one you will be billed for. See §4.
- If the active model is your *worker* model and not your *strong* model, the main session has fallen back. From this point the rule is: **do not spawn another same-model worker for serial work.** A short status note ("main fell back to <worker-model>") in your context is enough; the rule then applies automatically.
...
- The runtime's worker report (if exposed) should list the active model ID.
...
The model *name* you see in the runtime, in headers, in `session_status`, or in the worker's `Model:` field, identifies **which model produced the output**. It does **not** always identify **which model you are billed for**.
...
- [ ] I know which model the main session is currently on (from `session_status` or equivalent).
...
- [ ] If two consecutive attempts failed, I changed strategy (delegated if on strong; isolated a subproblem if already on worker), instead of trying a third time the same way.
...
- **Main and worker are the same model and the work was serial**: that was a cost-doubling event. The skill was not applied. Decide whether the workflow warrants a second look.
